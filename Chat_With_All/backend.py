...
logging.basicConfig(
    level=logging.INFO,
    handlers=[
        logging.FileHandler("/KJ/RAG-APP/Chat_With_All/logs/chat_app.log"),
...
for directory in ['/KJ/RAG-APP/Chat_With_All/logs', '/KJ/RAG-APP/Chat_With_All/vector_store', '/KJ/RAG-APP/Chat_With_All/uploads', '/KJ/RAG-APP/Chat_With_All/static', '/KJ/RAG-APP/Chat_With_All/templates']:
...
def setup_wikipedia_agent(model_name, session_id):
    ...
    try:
        ...
        wikipedia_wrapper = WikipediaAPIWrapper(lang="en", top_k_results=3)
        
        def wikipedia_search(query):
            """Wrapper function to handle Wikipedia searches with validation."""
            ...
            try:
                ...
                if "srsearch" in error_msg:
                    return "Error: Please provide a specific search term to query Wikipedia."
                return f"Error querying Wikipedia: {error_msg}"
        
        tools = [
            Tool(
                name="Wikipedia",
                func=wikipedia_search,
                description="Useful for querying information from Wikipedia. Input should be a specific search query."
            )
        ]
...
def extract_text_from_pdf(file_path):
    ...
    try:
        ...
        with open(file_path, 'rb') as file:
            ...
            if not text.strip():
                logger.info("No text extracted directly, attempting OCR...")
                try:
                    ...
                    if not text.strip():
                        logger.error("No text could be extracted even with OCR")
                        return "Error: No text could be extracted from the PDF, even with OCR. The file might be corrupted or empty."
                except Exception as ocr_error:
                    logger.error(f"Error during OCR processing: {str(ocr_error)}")
                    return f"Error during OCR processing: {str(ocr_error)}"
...
def get_models():
    models = ['qwen2.5:1.5b','gemma3:1b', 'codellama', 'llama3.2']
...
def init_database():
    ...
    filename = f"/RAG-APP/Chat_With_All/uploads/{int(time.time())}_{file.filename}"
    file.save(filename)
    
    result = setup_database_agent(states[session_id]['model'], filename, session_id)
    return jsonify(result)
...
def chat():
    ...
    with open(os.path.join('/RAG-APP/Chat_With_All/logs', f'queries_{datetime.now().strftime("%Y%m%d")}.json'), 'a') as f:
